"""Thread-safe round-robin rotation across up to 7 Groq API keys.
...
-> Key5 -> Key6 -> Key7 -> Key1 ..., never randomized, so distribution across keys
stays even and predictable.

Only whichever of LLM_API_KEY / LLM_API_KEY_2 / LLM_API_KEY_3 / LLM_API_KEY_4 /
LLM_API_KEY_5 / LLM_API_KEY_6 / LLM_API_KEY_7 are actually non-empty participate —
an environment with only LLM_API_KEY set (the pre-rotation default) still works
exactly as before, just without anything to rotate across.
"""
import itertools
import threading
